...
class Misp:
    ...
    def __init__(self):
        self.misp_backend = pymisp.PyMISP(
            OCD_DTL_MISP_HOST,
            OCD_DTL_MISP_API_KEY,
        ...
        self.thread_pool = concurrent.futures.ThreadPoolExecutor(
            max_workers=OCD_DTL_MISP_WORKER
        )

    @ignore(
        Exception, message="Skipping event insertion in misp since an error occurred"
    )
    ...
    def add_events(self, events: Iterable):
        ...
        results = []
        futures = [
            self.thread_pool.submit(self.add_event, event=event) for event in events
        ]
